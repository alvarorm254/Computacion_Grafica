...
public:
	...
	virtual void init(std::vector<Particle>& particles, solverParams* sp) {
		sp->deltaT = 5e-5f;
		sp->radius = 0.017f;
		sp->compression = 0.019f;//0.019
		sp->stretch = 0.0075f;//0.0075
		sp->hardening = 13.0f;//15
		sp->young = 12e4f;//4.8e4
		sp->poisson = 0.2f;//0.2
		sp->alpha = 0.95f;
		sp->density = 100.0f;//100
